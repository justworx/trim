...
class Server(sockserv, Runner):
	...
	def __init__(self, config=None, **k):
		...
		Pass config params for Runner as well as Server if you want, as 
		an example, to change the "sleep" time between the handling of 
		operations.
		
		Pass config as a dict, url, address, or port, or tuple 
		(host,port), as required by the sockinfo.listen() classmethod.
		
		As always, keyword arguments replace any matching key values given
		in a config dict, or in config generated by non-dict arguments.
	...
	def handlers(self):
		"""
		Returns a list of active handler objects. Response handlers in 
		this list are connected to a remote socket.
	...
	def remove(self):
		...
		or are removed for whatever reason.
		
		Call `remove` to retrieve a list of items that are scheduled to 
		be removed on the next pass through the event loop.
		
		If items linger here for longer than `Server.sleep` amount of
	...
	def query(self):
		"""
		Return a proplist containing queries, replies, and errors.
		
		>>> from trix.net.server import *
		>>> from trix.net.connect import *
		>>> s = Server(5555).starts()
		>>> c = Connect(5555)
		>>> c.write("Test")
		4
		>>> c.read()
		...
		try:
			...
			return trix.propx(dict(reply=r, error=e, ex=str(ex)))
