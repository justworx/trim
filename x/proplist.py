...
class proplist(propseq):
	...
	# THIS IS HARD!
	#
	def diff_items(self, *a):
		#
		# Build a grid with 2 or more columns; the first column is a list
		# of all items in `self.o`. Matching items from lists passed as 
		# additional argumets will appear beside the first column.
		#
		# Returns `proplist.propgrid`.
		#
		...
		# loop through each item in this object's list
		for o in self.o:
			
			row = []
			
			#
			# we're building a list of lists, each of which reflects the
			# presence (or absence) of `o` in the additional lists given
			# as argument *a.
			#
			
			#
			# The first item in the result list is the original list, 
			# self.o, whose items are the unique set of all items in all
			# lists given by *a. 
			#
			# The task here is to determine what differences between the
			# contents of each of the lists in *a.
			#
			row.append(o)
			
			# [ [first item in self.o] ]...
			
			# loop through each list in *a
			for alist in a:
				
				# now append o to each list that contains `o`, else None
				if o in alist:
					row.append(o)
				else:
					row.append(None)
