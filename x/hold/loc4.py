...
class Locale(object):
	...
	def day(self, dayNum):
		"""Pass the day number, 1-7. Returns day name."""
		return self.__day[dayNum-1]
	
	def abday(self, dayNum):
		"""Pass the day number, 1-7. Returns abbreviated day name."""
		return self.__abday[dayNum-1]
	
	def mon(self, monthNum):
		"""Pass the 1-based month number. Returns month name."""
		return self.__mon[monthNum-1]
	
	def abmon(self, monthNum):
		"""
		Pass the 1-based month number. Returns abbreviated month name.
		"""
		return self.__abmon[monthNum-1]
	...
	def datetime(self, f=None):
		return time.strftime(f or self.__f_dt)
	
	def date(self, f=None):
		return time.strftime(self.__f_d)
	
	def time(self, f=None):
		return time.strftime(self.__f_t)
	
	def ampm(self, f=None):
		return time.strftime(self.__fapm)
	
	def era(self, f=None):
		return time.strftime(self.__fera)
