...
assert(compact(b"abc") == b'eJxLTEoGAAJNASc=')
assert(expand(b"eJxLTEoGAAJNASc=") == b'abc')


#
# Now let's get Jiggy with some hairy unicode arguments.
#
assert(compact("abc") == b'eJxLTEoGAAJNASc=')

assert(b64.encode("abc") == b'YWJj')
assert(b64.decode("YWJj") == b'abc')
assert(b64.sencode("abc") == b'YWJj')
assert(b64.sdecode("YWJj") == b'abc')
assert(b64.uencode("abc") == b'YWJj')
assert(b64.udecode("YWJj") == b'abc')
assert(b32.encode("abc") == b'MFRGG===')
assert(b32.decode('MFRGG===') == b'abc')
assert(b16.encode("abc") == b'616263')
assert(b16.decode('616263') == b'abc')

assert(hex.encode("abc") == b"616263")
assert(hex.decode("616263") == b"abc")

assert(zlib.encode("abc") == b"x\x9cKLJ\x06\x00\x02M\x01'")
assert(zlib.decode("x\x9cKLJ\x06\x00\x02M\x01'") == b"abc")

assert(bz2.encode("abc") == b'BZh91AY&SYd\x8c\xbbs\x00\x00\x00\x01\x008\x00 \x00!\x98\x19\x84aw$S\x85\t\x06H\xcb\xb70')
assert(bz2.decode('BZh91AY&SYd\x8c\xbbs\x00\x00\x00\x01\x008\x00 \x00!\x98\x19\x84aw$S\x85\t\x06H\xcb\xb70') == b"abc")

assert(compact("abc") == b'eJxLTEoGAAJNASc=')
assert(expand('eJxLTEoGAAJNASc=') == b'abc')
